		...
		>
			...
			<Tabs onChange={(index) => setTabIndex(index)}>
				...
				>
					...
					>
						<Box display={"flex"} alignItems={"center"} padding={"13px 5px"}>
							<Text
								fontSize={"16px"}
				...
				<Box bgColor={useColorModeValue("#F0F0F5", "#191919")} padding={"32px"}>
					<TabPanels>
						<TabPanel p="0px">
							<GenericBigTableComponent
								tableName={"DeFi Hot Contract"}
								thread={[
									"Blockchain|Contract Name",
									"No. of Users",
									"No. of Transactions",
									"Total Deposit",
									"Share",
		...
		<>
			{tableData.map((item, i) => {
				return (
					<TableRow
						key={i}
						blockchain={{ name: item[0], src: item[4] }}
						users={item[1]}
						calls={item[2]}
						feeconsumed={item[3]}
						share={item[5]}
