"use client"
import { Avatar, Box, Flex, Td, Text, Th, Tr, useColorMode, useColorModeValue } from "@chakra-ui/react";
...
const GovernanceTable = ({ }) => {
    ...
    return (
        <>
            <Box w={"100%"} borderRadius={"6px"} bg={useColorModeValue("#FFFFFF", "#202020")} borderColor={useColorModeValue("#F0F0F5", "#272727")} mb={"50px"}>
                <Box layerStyle={"spaceBetween"} p={"20px"} >
                    <Box layerStyle={"center"} gap={"5px"}>
                        <Text variant={"smallTableHeader"}>
                            Governance
                        </Text>
                        <Text variant={"h4"} color={useColorModeValue('#434347', '#A8ADBD')}>
                            | Proposals
                        </Text>
                    </Box>
                            ...
                            return (
                                <Box layerStyle={"flexColumn"} gap={"10px"}>
                                    <Box display={"flex"} gap={"20px"}>
                                        <Text variant={"h3"} color={"#8F8F8F"}> Start </Text>
                                        <Text variant={"h3"}> {item?.item?.Start} </Text>
                                    </Box>
                                    <Box display={"flex"} gap={"20px"}>
                                        <Text variant={"h3"} color={"#8F8F8F"}> End </Text>
                                        <Text variant={"h3"}> {item?.item?.End} </Text>
                                    </Box>
                                    <Box display={"flex"} gap={"20px"}>
                                        <Text variant={"h3"} color={"#8F8F8F"}> Votes </Text>
                ...
                <Flex>
                    ...
                    >
                        <Box
                            ...
                            mr={"4px"}
                            bgColor={
                                item?.State === 'active' ? '#62D845' : '#FF4848'
                            }
                        ></Box>
                        <Text
                            _dark={{
